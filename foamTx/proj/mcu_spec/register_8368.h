...
enum{
	FLD_ADC_REFERENCE	= 	BIT(0),
};
/////////// adc select [4:0] channel; [6:5] mode; [7] signed
#define reg_adc_chn_m_sel		REG_ADDR8(0x2c)
enum{
	FLD_ADC_INPUT_CH_SEL		= BIT_RNG(0,2),
	FLD_DIFF_SEL				= BIT_RNG(4,5),
};
...
enum{
	FLD_ADC_DONE_EDGE = 	BIT_RNG(6,7),
};

#define reg_adc_run			REG_ADDR8(0x35)
enum{
	FLD_ADC_RUN = 	BIT(7),
};

#define reg_adc_dat_val		REG_ADDR16(0x38)
enum{
	FLD_ADC_CONV_VAL = 	BIT_RNG(0,9),
};
...
enum{
	...
	FLD_ADC_CHN0_SAMP_RESOL = 	BIT_RNG(3,4),
	FLD_ADC_CHN0_SIGN =	        BIT(5),
};
