...
static inline void gpio_init(void){
	...
	reg_gpio_f_nrm = (((MSDO_FUNC == AS_GPIO) ? FLD_MSDO_FUNC : 0)				
		...
		| ((SWM_FUNC == AS_GPIO) ? FLD_SWM_FUNC: 0)
...
static inline void gpio_set_func(u32 pin, u32 func){
	...
	case GPIO_DO:
		if(AS_GPIO == func){
			BM_SET(reg_gpio_f_nrm, FLD_DO_FUNC);
		}else{
			BM_CLR(reg_gpio_f_nrm, FLD_DO_FUNC);
		}
		break;
	case GPIO_DI:
		if(AS_GPIO == func){
	...
	case GPIO_SWS:
		if(AS_GPIO == func){
			BM_SET(reg_gpio_f_nrm, FLD_SWS_FUNC);
		}else{
			BM_CLR(reg_gpio_f_nrm, FLD_SWS_FUNC);
		}
		break;

	case GPIO_SWM:
		if(AS_GPIO == func){
			BM_SET(reg_gpio_f_nrm, FLD_SWM_FUNC);
		}else{
			BM_CLR(reg_gpio_f_nrm, FLD_SWM_FUNC);
		}
		break;
	...
	case GPIO_DM:
		if(AS_GPIO == func){
			BM_SET(reg_gpio_f_nrm, FLD_DP_FUNC);
		}else{
			BM_CLR(reg_gpio_f_nrm, FLD_DP_FUNC);
		}
		break;
	case GPIO_GP22:
		if(AS_GPIO == func){
			BM_CLR(reg_gpio_f_nrm, FLD_UART_RX_FUNC);
		}else{
			BM_SET(reg_gpio_f_nrm, FLD_UART_RX_FUNC);
		}
		break;
	case GPIO_GP23:
		if(AS_GPIO == func){
			BM_CLR(reg_gpio_f_nrm, FLD_UART_TX_FUNC);
		}else{
			BM_SET(reg_gpio_f_nrm, FLD_UART_TX_FUNC);
		}
		break;

	case GPIO_GP24:
		if(AS_GPIO == func){
			BM_CLR(reg_gpio_f_nrm, FLD_SPWM0_FUNC);
		}else{
			BM_SET(reg_gpio_f_nrm, FLD_SPWM0_FUNC);
		}
		break;

	case GPIO_GP25:
		if(AS_GPIO == func){
			BM_CLR(reg_gpio_f_nrm, FLD_SPWM1_FUNC);
		}else{
			BM_SET(reg_gpio_f_nrm, FLD_SPWM1_FUNC);
		}
		break;
	case GPIO_GP26:
		if(AS_GPIO == func){
			BM_CLR(reg_gpio_f_nrm, FLD_SPWM2_FUNC);
		}else{
			BM_SET(reg_gpio_f_nrm, FLD_SPWM2_FUNC);
		}
		break;
	}
}
