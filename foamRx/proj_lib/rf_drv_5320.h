...
#define	RF_TX_PA_POWER_LOW		WriteAnalogReg (0x9b, 0x3f)
#define	RF_TX_PA_POWER_HIGH		WriteAnalogReg (0x9b, 0xbf)

void CalibTOS32M (void);

int rf_drv_init (int wakeup_from_suspend);
void RF_resume(void);
void SetRxMode (unsigned char chn, unsigned char set);
void SetTxMode (unsigned char chn, unsigned char set);
